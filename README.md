...
The dashboard combines particle-number concentration, synchronized SMPS size-distribution heatmaps, weather, wind roses, rain, official air-quality observations, and Pearson/Spearman correlation analysis. A dedicated profile tab shows selectable SMPS bar profiles with 95% confidence intervals for any date range, weekday, hour of day, and eight wind-direction sectors. Up to five groups can be compared at once. The overview compares hourly CPC and SMPS total concentrations directly. Use the site selector in the header to switch between Rome and Hamburg.
